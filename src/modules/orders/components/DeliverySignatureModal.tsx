...
export const DeliverySignatureModal = ({ isOpen, onClose, onConfirm, saving }: {
  ...
  return <Modal isOpen={isOpen} onClose={saving ? () => {} : onClose} title="Firma de recibido" size="lg">
    <form onSubmit={submit} className="space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-2"><span className="text-[13px] font-medium">Firma</span><button type="button" disabled={saving} onClick={clear} className="flex items-center gap-1 text-[13px] text-[#0066CC]"><Eraser className="w-4 h-4" />Borrar firma</button></div>
        <canvas ref={canvasRef} width={600} height={200} aria-label="Área para firmar de recibido"
          className="block w-full h-[180px] rounded-xl border border-gray-200 bg-white touch-none cursor-crosshair"
      ...
      <div className="flex justify-end gap-3 border-t border-gray-100 pt-4">
        <button type="button" disabled={saving} onClick={onClose} className="rounded-lg border border-gray-200 px-4 py-2 text-[13px]">Cancelar</button>
        <button type="submit" disabled={saving || !hasStroke || recipientName.trim().length < 2} className="flex items-center gap-2 rounded-lg bg-[#0066CC] px-4 py-2 text-[13px] font-medium text-white disabled:opacity-50"><PenLine className="w-4 h-4" />{saving ? 'Guardando entrega…' : 'Confirmar entrega'}</button>
      </div>
    </form>
  </Modal>;
};
